...
# Western's old target (wwccwy.peopleadmin.com) stopped resolving entirely
# (NXDOMAIN) -- confirmed 2026-08-02. The college's careers page now lives
# on schooljobs.com, the same NEOGOV platform already used by Casper
# College, Central Wyoming, and Gillette College (confirmed via matching
# "NEOGOV" generator markers in both pages' raw HTML). Selectors below
# mirror Casper's chunk, the closest working sibling on this same
# subdomain pattern, since Western's actual rendered DOM couldn't be
# verified directly in this environment (no Firefox/geckodriver here) --
# this should be spot-checked against the live site the first time it
# actually runs.
base_url <- "https://www.schooljobs.com/careers/westernwyoming"

# Start RSelenium. Pagination continues until a genuinely empty page
# (matching LCCC/Eastern Wyoming's pattern) instead of a hardcoded page
# count, and a page-level error is skipped rather than treated as the end
# of listings.
selenium_session <- start_selenium()
remDr <- selenium_session$remDr
  ...
  safe_scrape(
    ...
    scrape_fn = function() {
      extract_jobs <- function(page_url) {
        ...
        Sys.sleep(10)  # allow full page load
        ...
        # Posted date is already relative text on this platform (e.g. "3
        # days ago") -- keep as-is, matching Casper/Central/Gillette.
        posted_dates <- soup %>%
          html_nodes(".list-entry-starts span") %>%
          html_text(trim = TRUE)
        if (length(posted_dates) == 0) posted_dates <- rep(NA, length(job_titles))
      ...
      page <- 1
      repeat {
        page_url <- if (page == 1) base_url else paste0(base_url, "?page=", page)

        outcome <- tryCatch(
          list(df = extract_jobs(page_url), error = NULL),
          error = function(e) list(df = NULL, error = conditionMessage(e))
        )

        if (!is.null(outcome$error)) {
          message("Western Wyoming Community College: error scraping page ", page, ": ", outcome$error)
          page <- page + 1
          Sys.sleep(1)
          if (page > 20) break  # backstop against looping forever if the site is down
          next
        }
